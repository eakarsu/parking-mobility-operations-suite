const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4990';
...
const adminCookie = await login('admin@parking-mobility-operations.local', 'admin123');
const managerCookie = await login('manager@parking-mobility-operations.local', 'manager123');
const analystCookie = await login('analyst@parking-mobility-operations.local', 'analyst123');
